#!/bin/sh
	...
	if [ "$(head -1 /usr/share/adbyby/data/lazy.txt | awk -F' ' '{print $3,$4}')" == "2017-1-2 00:12:25" ];then
		while : ; do
			wget -T 3 -O /tmp/lazy.txt https://cdn.jsdelivr.net/gh/adbyby/xwhyc-rules/lazy.txt
			if [ "$?" == "0" ];then
		...
		while : ; do
			wget -T 3 -O /tmp/video.txt https://cdn.jsdelivr.net/gh/adbyby/xwhyc-rules/video.txt
			if [ "$?" == "0" ];then
	...
	if [ "$(uci -q get adbyby.@adbyby[0].wan_mode)" == "1" ];then
		...
		while : ; do
			wget -T 3 -O /tmp/adbyby/adbyby_adblock/dnsmasq.adblock https://cdn.jsdelivr.net/gh/small-5/adbyby/dnsmasq.adblock
			[ "$?" == "0" ] && break || sleep 2
		done

		while : ; do
			wget -T 3 -O /tmp/adbyby/adbyby_adblock/md5 https://cdn.jsdelivr.net/gh/small-5/adbyby/md5_1
			[ "$?" == "0" ] && break || sleep 2
		done
