...
if ! mount | grep adbyby >/dev/null 2>&1; then
	...
	if [ $wan_mode -eq 1 ]; then
		...
		wget -4 -t 9 -T 3 -O- https://dev.tencent.com/u/Small_5/p/adbyby/git/raw/master/easylistchina%2Beasylist.txt | grep ^\|\|[^\*]*\^$ | sed -e 's:||:address\=\/:' -e 's:\^:/0\.0\.0\.0:' > /tmp/dnsmasq.adblock || \
		wget -4 -O- https://easylist-downloads.adblockplus.org/easylistchina+easylist.txt | grep ^\|\|[^\*]*\^$ | sed -e 's:||:address\=\/:' -e 's:\^:/0\.0\.0\.0:' > /tmp/dnsmasq.adblock
