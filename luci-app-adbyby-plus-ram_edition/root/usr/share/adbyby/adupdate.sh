...
B=0
	...
	wget -O /tmp/md5.json https://cdn.jsdelivr.net/gh/adbyby/xwhyc-rules/md5.json || wget -O /tmp/md5.json https://raw.githubusercontent.com/adbyby/xwhyc-rules/master/md5.json
	if [ -s "/tmp/md5.json" ];then
		...
		if [ "$lazy_online"x != "$lazy_local"x ];then
			...
			wget -O /tmp/lazy.txt https://cdn.jsdelivr.net/gh/adbyby/xwhyc-rules/lazy.txt || wget -O /tmp/lazy.txt https://raw.githubusercontent.com/adbyby/xwhyc-rules/master/lazy.txt
		...
		if [ "$video_online"x != "$video_local"x ];then
			...
			wget -O /tmp/video.txt https://cdn.jsdelivr.net/gh/adbyby/xwhyc-rules/video.txt || wget -O /tmp/video.txt https://raw.githubusercontent.com/adbyby/xwhyc-rules/master/video.txt
